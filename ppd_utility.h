...
BOOLEAN add_to_item_list(struct ppd_list *, char *,char * ,char * ,int , int , int);

/*used to validate id during loading sequence*/
BOOLEAN id_validation(char *);

/*used to validate cent during loading sequence*/
BOOLEAN cent_validation(int, char *);

/*used to validate dollar during loading sequence*/
BOOLEAN dollar_validation(int, char *);

/*checks the amount of tokens per line*/
BOOLEAN coin_file_token_validation(char *, const char *);

/*checks the amount of tokens per line*/
BOOLEAN stock_file_token_validation(char *, const char *);

/*used to validate denomination*/
BOOLEAN denomination_validation(struct coin *, int, char *);

/*used to validate quantity*/
BOOLEAN quantity_validation(int, char *);

/*used to sort the coins into their respective array cell*/
void sort_coins(struct coin *, int, int);

/*Checks for duplicate id*/
BOOLEAN check_for_duplicate_id(struct ppd_list *, char *);
#endif
